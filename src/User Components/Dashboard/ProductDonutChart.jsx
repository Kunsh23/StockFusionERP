...
const ProductDonutChart = () => {
    ...
    axios.get('http://localhost:8080/api/products/my', { withCredentials: true })
      .then((response) => {
        ...
        const categoryQuantity = categories.map((category) => {
          ...
          return filteredProducts.reduce((sum, product) => sum + product.quantity, 0); // Sum of quantities for each category
        });
          ...
          datasets: [
            {
              ...
              backgroundColor: ['#FF6384', '#36A2EB', '#FFCE56', '#FF5733', '#C70039'], // Donut chart segment colors
              hoverBackgroundColor: ['#FF3A3A', '#33A2FF', '#FFCD33', '#FF5722', '#D50032'], // Hover colors
            },
          ],
        });
  ...
  return (
    <div>
      ...
      {chartData && <Doughnut data={chartData} />}  {/* Render Doughnut chart */}
    </div>
  );
};
