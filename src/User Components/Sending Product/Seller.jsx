...
const Buyer = ({ theme }) => {
  ...
  const colDefs = useMemo(
    () => [
      ...
      { field: "sentAt", headerName: "Send Date", flex: 1.1 },
  ...
  const handleAddProduct = async (e) => {
    ...
    //   alert(productname, category, quantity, price, vendor); // Debugging line
      ...
      const response = await axios.post(
        "http://localhost:8080/api/sending/add",
        { 
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:8080/api/sending/all",
          { withCredentials: true }
        );
        setRowData(response.data); // Assuming API returns an array of objects
        setFilteredData(response.data); // Set initial filtered data
            ...
            <div className="modal-content">
              <div className="modal-header justify-content-between">
                <span className="modal-title">
                  {t("Add a purchased item to your inventory")}
                </span>
                <button
                  type="button"
                  className="btn close-btn"
