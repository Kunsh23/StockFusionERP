...
const Stocks = ({ theme }) => {
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      alert(productname, category, quantity, price); // Debugging line
  ...
  const handleUpdateProduct = async (e) => {
    ...
    // Dynamically construct API endpoint with user-provided product ID
    const updateUrl = `http://localhost:8080/api/products/update/${productId}`;
    ...
    try {
      ...
      // Handle errors and set error state
      if (error.response) {
        setError(error.response.data || "Error updating product.");
      } else if (error.request) {
        setError("No response from server. Please try again later.");
      } else {
        setError("An unexpected error occurred.");
  ...
  const handleDeleteProduct = async (e) => {
    ...
    // Dynamically construct API endpoint with user-provided product ID
    const updateUrl = `http://localhost:8080/api/products/id/${productId}`;
    ...
    try {
      ...
      // Handle errors and set error state
      if (error.response) {
        setError(error.response.data || "Error deleted product.");
      } else if (error.request) {
        setError("No response from server. Please try again later.");
      } else {
        setError("An unexpected error occurred.");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:8080/api/products/my", { withCredentials: true });
          ...
          <div className="modal-dialog modal-box">
            <div className="modal-content">
              <div className="modal-header justify-content-between">
                <span className="modal-title">Export Stock Data</span>
                <button
                  type="button"
        ...
        >
          <div className="modal-dialog modal-box">
            <div className="modal-content">
              <div className="modal-header justify-content-between">
                <span className="modal-title">
                  Update an existing item from your inventory
                </span>
                <button
                  type="button"
                  className="btn close-btn"
